...
class _AdminAccountPageState extends State<AdminAccountPage> {
  ...
  Future<void> fetchUserDataFromFirestore(String email) async {
    try {
      CollectionReference usersCollection = FirebaseFirestore.instance.collection('admin');

      // Fetch the user data using the user's email
      QuerySnapshot querySnapshot = await usersCollection.where('adminEmail', isEqualTo: email).get();
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        setState(() {
          adminName = userData['adminName']?.toString() ?? '';
        });
      } else {
    ...
    return GestureDetector(
      onTap: () async {
        if (title == "Edit Profile") {
          Navigator.pushNamed(context, '/adminEditProfilePage');
