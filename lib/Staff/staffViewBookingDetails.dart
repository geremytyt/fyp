...
class StaffViewBookingDetailsPage extends StatelessWidget {
  final String bookingID;
  final String poiID;

  StaffViewBookingDetailsPage({required this.bookingID, required this.poiID});
  ...
  Future<void> updateBookingStatus(String status) async {
    try {
      await FirebaseFirestore.instance.collection('booking').doc(bookingID).update({'bookingStatus': status});
                        ...
                        if (paymentSnapshot.connectionState == ConnectionState.waiting) {
                          ...
                          return Column(
                            ...
                            children: [
                              ...
                              // Display additional information and buttons for 'refund' status
                              if (bookingDetails['bookingStatus'] == 'refund')
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    SizedBox(height: 16.0),
                                    ...
                                    Container(
                                      ...
                                      child: Padding(
                                        ...
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Text('Refund Request', style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold)),
                                            Text('User ID: ${bookingDetails['userID']}'),
                                            Text('Refund Request: ${bookingDetails['refundRequest'] ?? 'N/A'}'),
                                          ],
                                        ),
                                      ),
                                    ),
                                      ...
                                      children: [
                                        ElevatedButton(
                                          onPressed: () async {
                                            ...
                                            await updateBookingStatus('refunded');
                                          },
                                          child: Text('Approve'),
                                        ),
                                        ElevatedButton(
                                          onPressed: () async {
                                            // Update booking status to 'refundRejected'
                                            await updateBookingStatus('refundRejected');
                                          },
